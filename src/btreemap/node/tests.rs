...
mod mem_size_tests {
    ...
    #[test]
    fn lazy_value_by_value() {
        // ByVal with a 5-byte blob: reports the serialized length.
        let val = LazyValue::by_value(vec![1, 2, 3, 4, 5]);
        assert_eq!(val.mem_size(), 5);
    }

    #[test]
    fn lazy_value_by_ref_not_loaded() {
        // ByRef before loading: offset (8) + size (4) = 12, no data.
        let val = LazyValue::by_ref(Bytes::new(100), 42);
        assert_eq!(val.mem_size(), 8 + 4);
    }

    #[test]
    fn lazy_value_by_ref_loaded() {
        // ByRef after loading: offset (8) + size (4) + data (42) = 54.
        let val = LazyValue::by_ref(Bytes::new(100), 42);
        val.get_or_load(|_, size| vec![0u8; size as usize]);
        assert_eq!(val.mem_size(), 8 + 4 + 42);
    }

    #[test]
    fn lazy_key_u32_by_value() {
        // u32 serializes to 4 bytes.
        let key = LazyKey::<u32>::by_value(42);
        assert_eq!(key.mem_size(), 4);
    }

    #[test]
    fn lazy_key_u32_by_ref_not_loaded() {
        let key = LazyKey::<u32>::by_ref(Bytes::new(100), 4);
        assert_eq!(key.mem_size(), 8 + 4);
    }

    #[test]
    fn lazy_key_u32_by_ref_loaded() {
        let key = LazyKey::<u32>::by_ref(Bytes::new(100), 4);
        key.get_or_load(|_, _| 42u32);
        assert_eq!(key.mem_size(), 8 + 4 + 4);
    }

    #[test]
    fn lazy_key_vec_by_value() {
        // Vec<u8> key with 10 bytes of data.
        let key = LazyKey::<Vec<u8>>::by_value(vec![0u8; 10]);
        assert_eq!(key.mem_size(), 10);
    }

    #[test]
    fn lazy_key_vec_by_ref_not_loaded() {
        let key = LazyKey::<Vec<u8>>::by_ref(Bytes::new(100), 10);
        assert_eq!(key.mem_size(), 8 + 4);
    }

    #[test]
    fn lazy_key_vec_by_ref_loaded() {
        let key = LazyKey::<Vec<u8>>::by_ref(Bytes::new(100), 10);
        key.get_or_load(|_, size| vec![0u8; size as usize]);
        assert_eq!(key.mem_size(), 8 + 4 + 10);
    }
}
